async function fetchDataPlaces() {
    try {
        const response = await fetch('/stats_api'); // Flask endpoint
        const data = await response.json();
        
        const datared = data['container_places'][0]
        const datamid = data['container_places'][1]
        const datagreen = data['container_places'][2]
        ...
        createTable(datagreen, 6,"Низкий уровень риска", tableContainer)
